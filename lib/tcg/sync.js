...
import { notifyNewPosts, sendTelegramSyncSummary } from './notifications.js'
...
const normalizeApifyActorId = (value) => {
  const raw = getFirstString(value)
  if (!raw) return ''

  // Accept full console URLs and extract the actor id.
  const fromUrl = raw.match(/\/actors\/([^/?#]+)/i)
  const base = fromUrl ? decodeURIComponent(fromUrl[1]) : raw

  // Accept "username/actor-name" and convert to API id format "username~actor-name".
  if (base.includes('/') && !base.includes('~')) {
    const [owner, actor] = base.split('/', 2)
    if (owner && actor) {
      return `${owner}~${actor}`
    }
  }
...
const fetchFromApify = async (handles, postsPerHandle) => {
  ...
  const actorId = normalizeApifyActorId(getApifyActorId())
  if (!actorId) {
    throw new Error('Missing APIFY_ACTOR_ID for Apify scraping.')
  }
  const input = {
  ...
  const endpoint = `${APIFY_BASE_URL}/acts/${encodeURIComponent(actorId)}/run-sync-get-dataset-items?token=${encodeURIComponent(token)}&clean=true`
...
export const syncInstagramPosts = async (options = {}) => {
  ...
  const postNotificationResult = notify
    ? await notifyNewPosts(postsToInsert)
    : { sent: 0, errors: [] }
  const summaryNotificationResult = notify
  ...
  const notificationErrors = [
    ...(postNotificationResult.errors || []),
    ...(summaryNotificationResult.errors || []),
  ]
  ...
  return {
    ...
    fetched: normalizedPosts.length,
    inserted: postsToInsert.length,
    notified: (postNotificationResult.sent || 0) + (summaryNotificationResult.sent || 0),
    notificationErrors,
